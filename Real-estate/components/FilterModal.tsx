...
const PRICE_PRESETS = [
  { label: "Under ugx50Sh", min: null, max: 5000000 },
  { label: "ugx50L – ugx1M", min: 5000000, max: 10000000 },
  { label: "ugx1Cr – ugx2M", min: 10000000, max: 20000000 },
  { label: "Above ugx2M", min: 20000000, max: null },
];
    ...
    >
      <View className='flex-1 bg-gray-50'>
        ...
        >
          ...
          <View className='flew-row flex-wrap gap-2 mb-6'>
            {TYPES.map(item => (
              <TouchableOpacity
          ...
          <View className='flew-row flex-wrap gap-2 mb-6'>
            {BEDS.map(item => (
              <TouchableOpacity
                ...
                style={shadow}
              >
                <Text className={` text-sm font-bols &{chipText(bedrooms === item.value)}`}>{item.label}</Text>
              </TouchableOpacity>
            ))}
          </View>

          <Text className='text-base font-bold text-gray-800 mb-3'> Price Range (ugx)</Text>
          <View className='flex-row gap-3 mb-3'>
            {[
              ...
              <View key={label} className='flex-1'>
                <Text className='text-ts text-gray-500 mb-1.5 font-medium'>{label}</Text>
                <View className='flex-row items-center by-white rounded-2xl px-3 border border-gray-200' style={shadow}
                >
                  <Text className='text-gray-400 text-sm mr-1'>Ugx</Text>
                  <TextInput
            ...
            {PRICE_PRESETS.map((preset) => {
              ...
              return (
                <TouchableOpacity
                  key={preset.label}
                  onPress={() => {
                    setLocalMin(preset.min !== null ? String(preset.min) : '');
                    setLocalMax(preset.max !== null ? String(preset.max) : '');
                    setMinPrice(preset.min);
                    setMaxPrice(preset.max);
                  }}
                  className={`px-3 py-1.5 rounded-full border ${
                    active ? 'bg-blue-500 border-blue-300' : 'bg-white border-gray-200'
                  }`}
                >
                  <Text className={`text-sm font-semibold ${active ? 'text-blue-600' : 'text-gray-500'}`}>
                    {preset.label}
                  </Text>
                </TouchableOpacity>
              );
